...
def main():
    ...
    if st.button("Predict Base Price"):
        if selected_company == "Select Make" or (bike_companies[selected_company]["models"] and model_code is None):
            ...
            base_price = prediction([final, model_code or 0, company_code, float(cc)])
            ...
            new_bike = Bike(year=Year, month=Month, yeardiff=final, cc=cc, company=selected_company, model=selected_model if model_code else "N/A", predicted_price=base_price)
